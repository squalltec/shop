...
$joinQuery = "FROM `tbl_user_privilege` AS `u` JOIN `tbl_user` AS `ua` ON (`ua`.`idtbl_user` = `u`.`tbl_user_idtbl_user`) JOIN `tbl_menulist` AS `ub` ON (`ub`.`idtbl_menulist` = `u`.`tbl_menulist_idtbl_menulist`)";

if($_POST['usertype']==1){
    $extraWhere = "`u`.`status` IN (1, 2)";
}
else{
    $extraWhere = "`u`.`status` IN (1, 2) AND `u`.`tbl_user_idtbl_user`!=1";
}
