...
<div class="modal fade" id="modaltrack" data-backdrop="static" data-keyboard="false" tabindex="-1"
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formtracking" autocomplete="off">
...
<script>
        ...
        $('#dataTable tbody').on('click', '.btntracking', function() {
            ...
            $.ajax({
                ...
                success: function(result) { //alert(result);
                    ...
                    $('#trackingcode').val(obj.trackingno);
                    $('#trackingurl').val(obj.trackingweb);
        ...
        $('#dataTable').DataTable( {
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, full) {
                        var button='';
                        button+='<button class="btn btn-outline-dark btn-sm btnview mr-1" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="View invoice"><i class="far fa-eye"></i></button>';
                        if(full['status']==2){
                            button+='<button class="btn btn-secondary btn-sm btnreactive mr-1" id="'+full['idtbl_order']+'"><i class="fas fa-check"></i></button>';
                        }

                        if(full['callstatus']==1 && full['status']==1){
                            button+='<button class="btn btn-success btn-sm mr-1" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Call answered"><i class="fas fa-phone"></i></button>';
                        }
                        else if(full['callstatus']==0 && full['status']==1){
                            button+='<button class="btn btn-danger btn-sm mr-1 btncall" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Call not answered"><i class="fas fa-phone"></i></button>';
                        }

                        if(full['status']==1){
                            button+='<button class="btn btn-outline-secondary btn-sm btntracking mr-1" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Track order"><i class="fas fa-map-marker-alt"></i></button>'
                        }

                        if(full['acceptstatus']==0 && full['status']==1){button+='<button class="btn btn-outline-orange btn-sm btnaccept mr-1" data-toggle="tooltip" data-placement="bottom" title="Order not accepted" id="'+full['idtbl_order']+'"><i class="fas fa-times"></i></button>';}
                        else if(full['acceptstatus']==1 && full['status']==1){button+='<button class="btn btn-outline-success btn-sm mr-1" data-toggle="tooltip" data-placement="bottom" title="Order accepted"><i class="fas fa-check"></i></button>';}

                        if(full['paystatus']==0 && full['status']==1){button+='<button class="btn btn-outline-danger btn-sm mr-1 btnpayment" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Payment pending"><i class="fas fa-money-bill-alt"></i></button>';}
                        else if(full['status']==1){button+='<button class="btn btn-outline-success btn-sm mr-1 btnpaydone" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Payment completed"><i class="fas fa-money-bill-alt"></i></button>';}

                        if(full['shipstatus']==0 && full['status']==1){button+='<button class="btn btn-outline-danger btn-sm mr-1 btnship" data-toggle="tooltip" data-placement="bottom" title="Goods not shipped" id="'+full['idtbl_order']+'"><i class="fas fa-dolly"></i></button>';}
                        else if(full['status']==1){button+='<button class="btn btn-outline-success btn-sm mr-1" data-toggle="tooltip" data-placement="bottom" title="Goods Shipped"><i class="fas fa-dolly"></i></button>';}

                        if(full['deliverystatus']==0 && full['status']==1){button+='<button class="btn btn-outline-danger btn-sm mr-1 btndelivery" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Delivery pending"><i class="fas fa-truck"></i></button>';}
                        else if(full['status']==1){button+='<button class="btn btn-outline-success btn-sm mr-1" data-toggle="tooltip" data-placement="bottom" title="Goods delivered"><i class="fas fa-truck"></i></button>';}

                        if(full['deliverystatus']==0 && full['status']==1){button+='<button class="btn btn-outline-danger btn-sm mr-1 btncancel" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Cancel order"><i class="far fa-trash-alt"></i></button><button class="btn btn-primary btn-sm btnreturn" id="'+full['idtbl_order']+'" data-toggle="tooltip" data-placement="bottom" title="Goods returned"><i class="fas fa-redo-alt"></i></button>';}
                        return button;
                    }
                }
            ],
    ...
    function printcourier() {
        printJS({
            ...
            type: 'html',
            style: '@page { size: 100mm 120mm; portrait; margin:0.25cm; }',
            targetStyles: ['*']
        })
    }
