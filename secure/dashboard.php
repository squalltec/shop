...
$sqlorder="SELECT COUNT(*) AS `count` FROM `tbl_order` WHERE `shipstatus`=0 AND `deliverystatus`=0 AND MONTH(`orderdate`)='$thismonth'";
$resultorder=$conn->query($sqlorder);
$roworder = $resultorder-> fetch_assoc();

$sqlongoing="SELECT COUNT(*) AS `count` FROM `tbl_order` WHERE `shipstatus`=1 AND `deliverystatus`=0 AND MONTH(`orderdate`)='$thismonth'";
$resultongoing=$conn->query($sqlongoing);
$rowongoing = $resultongoing-> fetch_assoc();

$sqldelivered="SELECT COUNT(*) AS `count` FROM `tbl_order` WHERE `shipstatus`=1 AND `deliverystatus`=1 AND MONTH(`orderdate`)='$thismonth'";
$resultdelivered=$conn->query($sqldelivered);
$rowdelivered = $resultdelivered-> fetch_assoc();

$sqlnewcus="SELECT COUNT(*) AS `count` FROM `tbl_customer` WHERE `status`=1 AND MONTH(`updatedatetime`)='$thismonth'";
$resultnewcus=$conn->query($sqlnewcus);
$rownewcus = $resultnewcus-> fetch_assoc();
